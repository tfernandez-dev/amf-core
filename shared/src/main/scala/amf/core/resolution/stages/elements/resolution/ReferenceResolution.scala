...
class ReferenceResolution(links: mutable.Map[String, DomainElement] = mutable.Map(), keepEditingInfo: Boolean, modelResolver:Option[ModelReferenceResolver], errorHandler:ErrorHandler,customDomainElementTransformation:(DomainElement, Linkable) => DomainElement = (d:DomainElement, l:Linkable) => d) extends ElementStageTransformer[DomainElement] {
